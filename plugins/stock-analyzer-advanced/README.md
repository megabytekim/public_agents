...
KRX(한국거래소)가 2025년 12월 27일부터 **로그인 필수** 정책으로 변경되어, pykrx 라이브러리의 KRX 기반 함수들이 작동하지 않습니다.

**원인**: AI 봇 무단 데이터 수집으로 인한 서버 과부하 → KRX 보안 강화

**영향받는 기능** (2026-01-19 업데이트):
| 함수 | 상태 | 비고 |
|------|------|------|
| `get_market_ohlcv_by_date()` | ✅ 작동 | Naver 소스 |
| `get_market_ticker_name()` | ✅ 작동 | Naver 소스 |
| `get_market_ticker_list()` | ✅ 해결 | Naver fallback 추가 |
| `get_market_fundamental()` | ✅ 해결 | Naver fallback 검증 |
| `get_market_cap()` | ✅ 해결 | Naver fallback 추가 |
...
**관련 GitHub 이슈**:
- [#244](https://github.com/sharebook-kr/pykrx/issues/244) - KRX 로그인 필수화
